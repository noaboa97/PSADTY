function Set-CMImplicitUninstall {
    ...
    .PARAMETER RemoteMachine
        Specifies a remote machine if the script isn't run on the sccm machine.
    .EXAMPLE
    ...
    Param (
        ...
        [Parameter(Mandatory=$false,HelpMessage="Remote Machine",Position=2)]
...
'@
    ...
    Try {
        ...
        ForEach ($ApplicationAssigment in $ApplicationAssigments) {
            ...
            if($RemoteMachine){
                $AssignmentInfo = Get-CimInstance -Namespace "ROOT\SMS\site_$SiteCode" -ClassName 'SMS_ApplicationAssignment' -Filter "AssignmentID = $AssignmentID" -ComputerName $RemoteMachine
            }else{
            ...
            If ($ShouldProcess) {
                if($RemoteMachine){
                    $AssignmentInfo | Set-CimInstance -Property @{ AdditionalProperties = ($AdditonalPoperties.OuterXml); OfferFlags = $OfferFlagsValue } -ErrorAction 'Stop' -ComputerName $RemoteMachine
                }else{
        ...
        try{
            Remove-Module 'ConfigurationManager' -Force -ErrorAction Ignore
        }catch{
        }
